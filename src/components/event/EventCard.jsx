...
function EventCard({ event }) {
  ...
  return (
    <Link
      to={`/events/${event.id}`}
      onClick={handleClick}
      className="group relative flex flex-col bg-white rounded-3xl overflow-hidden shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-500 will-change-transform h-full"
    >
      {/* Image Container */}
      <div className="relative h-56 overflow-hidden">
        {event.image ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-brand via-orange-500 to-secondary transition-transform duration-700 group-hover:scale-105 opacity-90" />
        )}
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-white bg-white/20 backdrop-blur-md border border-white/30 shadow-sm">
            {event.category}
          </span>
        </div>
        ...
        <div className="absolute bottom-4 left-4">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/90 backdrop-blur-md shadow-lg transform transition-transform group-hover:scale-105">
