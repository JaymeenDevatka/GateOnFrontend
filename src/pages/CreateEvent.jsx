  ...
  return (
    <div className="container-page max-w-3xl">
      <div className="space-y-1 mb-5">
        <h1 className="text-xl font-semibold text-slate-900">Create an event</h1>
        <p className="text-sm text-slate-500">
          This mirrors the Eventbrite organizer flow but saves everything only in frontend state for
          now.
        </p>
      </div>
      ...
      >
        ...
        <div className="space-y-1 max-w-xs">
          <label className="text-xs font-medium text-slate-700">Base ticket price (₹)</label>
          <input
            type="number"
            min="0"
            value={form.price}
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-slate-700">Ticket types</label>
          <p className="text-[11px] text-slate-500">
            Configure multiple ticket tiers (General, VIP, Student, Early Bird) with price, sale
            window and capacity.
          </p>
          <div className="space-y-2">
            {tickets.map((t) => (
              ...
              >
                ...
                <input
                  ...
                  min="0"
                  value={t.price}
                  onChange={(e) => updateTicket(t.id, 'price', Number(e.target.value))}
                  placeholder="Price (₹)"
                  className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-brand"
                />
                <input
                  ...
                  min="1"
                  value={t.maxQuantity}
                  onChange={(e) => updateTicket(t.id, 'maxQuantity', Number(e.target.value))}
                  placeholder="Max qty"
                  className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-brand"
                />
                <div className="flex flex-col gap-1">
                  <input
                    value={t.saleStart}
                    onChange={(e) => updateTicket(t.id, 'saleStart', e.target.value)}
                  ...
                  <input
                    value={t.saleEnd}
                    onChange={(e) => updateTicket(t.id, 'saleEnd', e.target.value)}
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-slate-700">Online / live streaming</label>
        ...
        <div className="space-y-2">
          ...
          <div className="inline-flex rounded-full border border-slate-200 bg-slate-50 p-1 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-full ${
                form.status === 'draft' ? 'bg-slate-900 text-white' : 'text-slate-700'
              }`}
            >
              Draft
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-full ${
                form.status === 'published' ? 'bg-slate-900 text-white' : 'text-slate-700'
              }`}
            >
              Publish
            </button>
          ...
          <p className="text-[11px] text-slate-500">
            Draft events stay private in your dashboard; published ones are discoverable.
          </p>
        </div>
